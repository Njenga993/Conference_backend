...
const REQUIRED_ENV = [
  ...
  "ADMIN_PASSWORD",    // full access
  "STAFF_PASSWORD",    // check-in only
];
...
role: 'staff' has read-only + check-in access
========================================
*/
const requireAuth = (allowedRoles = ["admin", "staff"]) => {
  return (req, res, next) => {
    ...
    try {
      ...
      req.user = decoded; // { role: 'admin'|'staff', iat, exp }
      next();
    } catch (err) {
...
async function processSuccessfulPayment(participantId, reference) {
  await dbRun(
    `UPDATE participants SET paymentStatus = ?, paymentReference = ? WHERE id = ?`,
    ["paid", reference, participantId]
  );
  const participant = await dbGet(`SELECT * FROM participants WHERE id = ?`, [participantId]);
...
Two passwords from .env:
  ADMIN_PASSWORD → role: 'admin' (full access)
  STAFF_PASSWORD → role: 'staff' (read-only + check-in)
========================================
*/
app.post("/auth/login", loginLimiter, (req, res) => {
  ...
  const token = jwt.sign(
    { role },
    JWT_SECRET,
    { expiresIn: "8h" } // session lasts 8 hours — good for a conference day
  );
...
AUTH: VERIFY TOKEN
GET /auth/verify
Used by frontend on page load to check if
...
app.post("/initialize-payment", paymentInitLimiter, async (req, res) => {
  ...
  const existing = await dbGet(
    `SELECT id FROM participants WHERE email = ? AND registrationType = ? AND paymentStatus = 'paid'`,
  ...
  try {
    await dbRun(
      ...
      (id, fullName, email, phone, country, organization, registrationType,
       excursion, galaDinner, amount, paymentStatus, createdAt)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        participantId, name, email.toLowerCase().trim(),
        metadata?.phone || "", metadata?.country || "",
        metadata?.organization || "", registrationType,
        metadata?.excursion ? 1 : 0, metadata?.galaDinner ? 1 : 0,
    ...
    await dbRun(`UPDATE participants SET paymentStatus = ? WHERE id = ?`, ["failed", participantId]).catch(() => {});
...
app.post("/verify-payment", verifyLimiter, async (req, res) => {
  ...
  try {
    ...
    if (data.status && data.data.status === "success") {
      ...
      const existing = await dbGet(`SELECT paymentStatus FROM participants WHERE id = ?`, [participantId]);
...
app.post("/paystack-webhook", async (req, res) => {
  ...
  catch { console.error("❌ Failed to parse webhook"); return; }
...
app.get("/ticket/:participantId", async (req, res) => {
  ...
  try {
    const participant = await dbGet(
      `SELECT * FROM participants WHERE id = ? AND paymentStatus = 'paid'`, [safeId]
    );
    ...
    res.sendFile(ticketPath, (err) => {
      if (err && !res.headersSent) res.status(404).json({ error: "Ticket not found" });
    });
  } catch (err) {
    if (!res.headersSent) res.status(500).json({ error: "Could not load ticket" });
  }
});
...
app.get("/ticket-download/:participantId", async (req, res) => {
  ...
  try {
    ...
    if (!res.headersSent) res.status(500).json({ error: "Could not download ticket" });
  }
});

/*
========================================
VERIFY TICKET (public — for check-in page)
GET /verify-ticket/:participantId
No auth needed — just returns participant info
========================================
*/
app.get("/verify-ticket/:participantId", async (req, res) => {
  ...
  try {
    const participant = await dbGet(
      `SELECT id, fullName, email, country, organization, registrationType,
              excursion, galaDinner, amount, paymentStatus,
    ...
    if (participant.paymentStatus !== "paid") {
      return res.json({ valid: false, error: "Payment not confirmed", paymentStatus: participant.paymentStatus });
    }
...
========================================
CHECK IN
PATCH /checkin/:participantId
Requires: staff or admin token
========================================
*/
app.patch("/checkin/:participantId", requireAuth(["admin", "staff"]), async (req, res) => {
  ...
  try {
    const participant = await dbGet(
      `SELECT id, fullName, paymentStatus, checkedIn FROM participants WHERE id = ?`, [safeId]
    );
    ...
    await dbRun(
      `UPDATE participants SET checkedIn = 1, checkedInAt = ?, checkedInBy = ? WHERE id = ?`,
      [new Date().toISOString(), staffName, safeId]
    );
...
app.get("/admin/participants", adminLimiter, requireAuth(["admin"]), async (req, res) => {
  try {
    ...
    const params = [];
    const conditions = [];
    if (status) { conditions.push(`paymentStatus = ?`); params.push(status); }
    if (type)   { conditions.push(`registrationType = ?`); params.push(type); }
...
(staff can see stats on the check-in counter)
========================================
*/
app.get("/admin/stats", adminLimiter, requireAuth(["admin", "staff"]), async (req, res) => {
  try {
    const [total, paid, pending, failed, checkedIn, byType, revenue] = await Promise.all([
      ...
      dbGet(`SELECT COUNT(*) as count FROM participants WHERE checkedIn = 1`),
    ...
    res.json({
      registrations: {
        total: total.count, paid: paid.count,
        pending: pending.count, failed: failed.count,
...
app.patch("/admin/undo-checkin/:participantId", adminLimiter, requireAuth(["admin"]), async (req, res) => {
  const safeId = path.basename(req.params.participantId);
  try {
    const participant = await dbGet(`SELECT id, fullName FROM participants WHERE id = ?`, [safeId]);
    if (!participant) return res.status(404).json({ error: "Participant not found" });
    await dbRun(
      `UPDATE participants SET checkedIn = 0, checkedInAt = NULL, checkedInBy = NULL WHERE id = ?`, [safeId]
    );
