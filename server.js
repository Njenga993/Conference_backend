// server.js - FULLY WORKING with Multi-Currency, BCC Emails, and Proper Auth
...
Fetches live USD to KES rate, with fallback and caching
========================================
*/
let cachedRate = null;
...
const getUSDToKESRate = async () => {
  ...
  if (
    ...
    console.log(
      `💱 Using cached exchange rate: 1 USD = ${cachedRate.toFixed(2)} KES`,
    );
    return cachedRate;
  }
  ...
  const apis = [
    {
      name: "Frankfurter",
      url: "https://api.frankfurter.app/latest?amount=1&from=USD&to=KES",
      getRate: (data) => data.rates?.KES,
    },
    {
  ...
  for (const api of apis) {
    try {
      console.log(`💱 Attempting to fetch rate from ${api.name}...`);
      const response = await axios.get(api.url, { timeout: 5000 });
      const rate = api.getRate(response.data);

      if (rate && rate > 0 && !isNaN(rate)) {
        cachedRate = rate;
        cacheTimestamp = Date.now();
        console.log(
          `✅ Live exchange rate from ${api.name}: 1 USD = ${rate.toFixed(2)} KES`,
        );
        return rate;
      }
    } catch (err) {
  ...
  console.log(`⚠️ Using fallback exchange rate: 1 USD = ${fallbackRate} KES`);
  return fallbackRate;
};
  ...
  try {
      ...
      return (req, res, next) => {
        ...
        try {
          ...
          if (!allowedRoles.includes(decoded.role)) {
            return res
              .status(403)
              .json({ error: "Access denied for your role." });
          }
          req.user = decoded;
          next();
        } catch (err) {
    ...
    */
    async function processSuccessfulPayment(participantId, reference) {
      ...
      if (!participant) {
        console.error(
          `❌ [ERROR] Participant not found in database: ${participantId}`,
        );
      ...
      console.log(
        `   - Current paymentreference: "${participant.paymentreference}"`,
      );
      ...
      if (!updated) {
        console.error(
          `❌ [CRITICAL ERROR] Participant disappeared after UPDATE!`,
        );
      ...
      console.log(
        `   - paymentreference is now: "${updated.paymentreference}"`,
      );
      ...
      sendTicketEmail(participantForTicket, ticketPath)
        ...
        .catch((err) => {
          console.error(
            `⚠️ Email send failed (async, non-blocking): ${err.message}`,
          );
          console.error(
            `   Participant: ${participant.email} | ID: ${participantId}`,
          );
        });
    ...
    */
    ...
    /*
    ========================================
    INITIALIZE PAYMENT - WITH MULTI-CURRENCY
    ========================================
    */
    app.post("/initialize-payment", paymentInitLimiter, async (req, res) => {
      ...
      console.log(
        `💰 Amount: ${amount} ${metadata?.currency || "USD"} for ${name} (${email})`,
      );

      if (!email || !amount || !name) {
        return res
          .status(400)
          .json({ error: "email, amount, and name are required" });
      }
      ...
      if (existing) {
        return res.status(409).json({
          error:
            "A paid registration already exists for this email and registration type.",
      ...
      try {
        ...
        if (selectedCurrency === "KES") {
          ...
          console.log(
            `💱 Currency conversion: ${amount} ${selectedCurrency} × ${exchangeRate} = ${amountInKES.toFixed(2)} KES`,
          );
        }
        ...
        console.log(
          `💳 Paystack amount: ${amountInKobo} kobo (${amountInKES.toFixed(2)} KES)`,
        );
        ...
        console.log(
          `   Amount in DB: ${Math.round(amountInKES)} KES (from ${amount} ${selectedCurrency})`,
        );
    ...
    */
    app.post("/verify-payment", verifyLimiter, async (req, res) => {
      ...
      try {
        ...
        console.log(
          `Paystack response status: ${data.status}, transaction status: ${data.data?.status}`,
        );
        ...
        if (data.status && data.data.status === "success") {
          ...
          if (!participantId) {
            console.error(
              "❌ Paystack response missing participantId in metadata",
            );
            return res
              .status(500)
              .json({
                error:
                  "Payment verification failed: Missing participant information",
              });
          }
          ...
          const existing = await dbGet(
            `SELECT paymentstatus FROM participants WHERE id = $1`,
            [participantId],
          );
          if (!existing) {
            console.error(
              `❌ Participant not found in database: ${participantId}`,
            );
            return res
              .status(404)
              .json({
                error: "Participant not found. Please contact support.",
              });
          }
    ...
    */
    app.get("/ticket/:participantId", async (req, res) => {
      ...
      try {
        const participant = await dbGet(
          `SELECT * FROM participants WHERE id = $1 AND paymentstatus = 'paid'`,
          [safeId],
        );
        if (!participant)
        ...
        res.sendFile(ticketPath, (err) => {
          if (err && !res.headersSent) {
            console.error("Error sending ticket file:", err.message);
    ...
    */
    app.get("/ticket-download/:participantId", async (req, res) => {
      ...
      try {
        ...
        const ticketPath = await ensureTicketExists(participantForTicket);
        res.download(ticketPath, `conference-ticket-${safeId}.pdf`, (err) => {
          if (err && !res.headersSent) {
        ...
        if (!res.headersSent)
          res.status(500).json({ error: "Could not download ticket" });
      }
    });
    ...
    */
    app.patch(
      ...
      async (req, res) => {
        ...
        try {
          const participant = await dbGet(
            `SELECT id, fullname, paymentstatus, checkedin FROM participants WHERE id = $1`,
            [safeId],
          );
          if (!participant)
          ...
          if (participant.paymentstatus !== "paid")
            return res
              .status(400)
              .json({ error: "Cannot check in — payment not confirmed" });
          if (participant.checkedin)
            return res
              .status(409)
              .json({ error: "Already checked in", alreadyCheckedIn: true });
          await dbRun(
            `UPDATE participants SET checkedin = $1, checkedinat = $2, checkedinby = $3 WHERE id = $4`,
            [true, new Date().toISOString(), staffName, safeId],
          );
    ...
    app.get(
      ...
      async (req, res) => {
        try {
          ...
          const conditions = [];
          if (status) {
            conditions.push(`paymentstatus = $${params.length + 1}`);
            params.push(status);
          }
          if (type) {
            conditions.push(`registrationtype = $${params.length + 1}`);
            params.push(type);
          }
          ...
          console.error("Admin participants error:", err.message);
    ...
    */
    app.post(
      "/admin/resend-ticket/:participantId",
      adminLimiter,
      requireAuth(["admin"]),
      async (req, res) => {
        const safeId = path.basename(req.params.participantId);
        try {
          ...
          if (!participant)
            return res
              .status(404)
              .json({
                error: "Participant not found or payment not confirmed",
              });
          ...
          sendTicketEmail(participantForTicket, ticketPath)
            .then(() =>
              console.log("📧 Resent ticket email to:", participant.email),
            )
            .catch((err) =>
              console.error("⚠️ Failed to resend email:", err.message),
            );

          res.json({
            success: true,
            message: "Ticket resent to " + participant.email,
          });
        } catch (error) {
          console.error("Resend ticket error:", error.message);
          res.status(500).json({ error: "Failed to resend: " + error.message });
        }
      },
    );
    ...
    */
    app.get(
      ...
      async (req, res) => {
        try {
          const [total, paid, pending, failed, checkedIn, byType, revenue] =
            await Promise.all([
              dbGet(`SELECT COUNT(*) as count FROM participants`),
              dbGet(
                `SELECT COUNT(*) as count FROM participants WHERE paymentstatus = 'paid'`,
              ),
              dbGet(
                `SELECT COUNT(*) as count FROM participants WHERE paymentstatus = 'pending'`,
              ),
              dbGet(
                `SELECT COUNT(*) as count FROM participants WHERE paymentstatus = 'failed'`,
              ),
              dbGet(
                `SELECT COUNT(*) as count FROM participants WHERE checkedin = $1`,
                [true],
              ),
              dbAll(
                `SELECT registrationtype, COUNT(*) as count FROM participants WHERE paymentstatus = 'paid' GROUP BY registrationtype`,
              ),
              dbGet(
                `SELECT SUM(amount) as total FROM participants WHERE paymentstatus = 'paid'`,
              ),
            ]);
          res.json({
            registrations: {
            ...
            byType: byType.reduce((acc, row) => {
              acc[row.registrationtype || "unknown"] = row.count;
              return acc;
            }, {}),
    ...
    */
    app.patch(
      "/admin/undo-checkin/:participantId",
      adminLimiter,
      requireAuth(["admin"]),
      async (req, res) => {
        const safeId = path.basename(req.params.participantId);
        try {
          const participant = await dbGet(
            `SELECT id, fullname FROM participants WHERE id = $1`,
            [safeId],
          );
          if (!participant)
            return res.status(404).json({ error: "Participant not found" });
          await dbRun(
            `UPDATE participants SET checkedin = $1, checkedinat = NULL, checkedinby = NULL WHERE id = $2`,
            [false, safeId],
          );
          console.log(`✅ Check-in reversed for: ${participant.fullname}`);
          res.json({
            success: true,
            message: `Check-in reversed for ${participant.fullname}`,
          });
        } catch (err) {
          console.error("Undo check-in error:", err.message);
          res.status(500).json({ error: err.message });
        }
      },
    );
    ...
    */
    app.listen(PORT, () => {
      ...
      console.log(
        `💰 All payments processed in KES (Paystack native currency)`,
      );
      console.log(
        `💱 Exchange rate cache: ${CACHE_DURATION / 1000 / 60} minutes`,
      );
      console.log(
        `📧 Email BCC enabled: ${process.env.SMTP_BCC || "not configured"}`,
      );
    });
  } catch (error) {
